	...
	const MovieEventHandlerDictionary &h)
{
	...
	shared_ptr<Movie> movie = make_shared<Movie>(
		lwf, this, movieId, -1, 0, 0, true, &handlers);
	...
	string linkageName, string aName, int aDepth, bool reorder)
{
	...
	return AttachMovie(linkageName, aName, aDepth, reorder, h);
}
...
void Movie::AttachLWF(shared_ptr<LWF> attachLWF, string aName,
	int aDepth, bool reorder, DetachHandler detachHandler)
{
